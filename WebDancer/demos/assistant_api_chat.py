"""An API-based WebDancer agent implementation"""
...
def init_api_agent_service(model: str = 'web-dancer'):
    """Initialize the API-based agent service"""
...
def init_api_search_agent_service(name: str = 'WebDancer-API', desc: str = 'API版本', reasoning: bool = True, max_llm_calls: int = 20, tools = ['search', 'visit'], addtional_agent = None):
    """Initialize the API-based search agent service"""
    llm_cfg = TextChatAtOAI({
        'model': 'web-dancer',
        'model_type': 'oai',
        'model_server': os.getenv('OPENAI_API_BASE', 'https://oneapi.hkgai.net/v1'),
    ...
    bot = SearchAgent(
        ...
        function_list=tools,
        system_message="",
        name=f'WebDancer-API',
        description=f"I am WebDancer API, a web information seeking agent, welcome to try!",
        extra={
            'reasoning': reasoning,
...
def app_tui():
    ...
    search_bot_api = init_api_search_agent_service()
    ...
    while True:
        ...
        response = []
        response_plain_text = ''
        for response in search_bot_api.run(messages=messages):
